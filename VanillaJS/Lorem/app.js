...
form.addEventListener('submit', function(e){
  e.preventDefault(); //i form di default inviano i dati ad un server, in questo modo evito questo comportamento

  const value = parseInt(amount.value); //tutti gli input ritornano delle stringhe, volendo in questo caso dei numeri bisogna convertirli

  //se non viene inserito nessun numero, un numero negativo o un numero maggiore di 9 mostro sempre un solo paragrafo a caso
  const random = Math.floor(Math.random() * text.length);
  if(isNaN(value) || value < 0 || value > 9){
    result.innerHTML = `<p class="result">${text[random]}</p>`
  }
  //se viene inserito un numero valido (tra 0 e 9)
  else{
    let tempText = text.slice(0, value);
    //trasformo tutte le stringhe in elementi paragrafo in modo da poter poi fare un join e ottenere un unico paragrafo e usare innerHTML
